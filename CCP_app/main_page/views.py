...
class ExhibitorRegisterView(FormView):
    template_name = "exhibitor_register.html"
...
class UserDetailsView(LoginRequiredMixin, FormView):
    ...
    def generate_registration_number():
        ...
        last_user = (
            User.objects.filter(
                registration_number__startswith=f"CCP/M/{current_year}/"
            )
...
class ReplyOrDetailMessageView(LoginRequiredMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        html_content = render_to_string(
            "email.html",
            {"title": title, "content": content, "original_message": original_message},
        )
        if send_mail(
            title,
            "",
...
def get_parent_pig_info(request):
    if request.method == "POST":
        ...
        selected_pig = get_object_or_404(
            Pig, name=selected_pig_name, nickname=selected_pig_nickname
        )
        pig_info = {
...
def get_pig_info(request):
    if request.method == "POST":
        selected_pig_name = (
            request.POST.get("pig_name").strip()
            if request.POST.get("pig_name")
            else None
        )
        selected_pig_nickname = (
            request.POST.get("pig_nickname").strip()
            if request.POST.get("pig_nickname")
            else None
        )
        selected_pig = Pig.objects.filter(
            name=selected_pig_name, nickname=selected_pig_nickname
        ).first()
        if selected_pig is not None:
            ...
            return JsonResponse(
                {"message": "Pig with the given name and nickname doesn't exist"}
            )
    else:
        return JsonResponse({"error": "Invalid request method"})
...
def breeder_add_pig(request):
    ...
    all_male_pigs = [
        f"{pig.name}   {pig.nickname}"
        for pig in Pig.objects.filter(sex="Male").exclude(nickname__exact="")
    ]
    all_female_pigs = [
        f"{pig.name}   {pig.nickname}"
        for pig in Pig.objects.filter(sex="Female").exclude(nickname__exact="")
    ]
    if request.method == "POST":
        pig_form = ExhibitorAddPigForm(request.POST, prefix="pig")
        parent_forms = [
            ExhibitorAddParentPigForm(request.POST, prefix=f"parent_{i}")
            for i in range(14)
        ]
        ...
        if pig_form.is_valid() and all(form.is_valid() for form in parent_forms):
            user_breeding = (
                Breeding.objects.filter(owners=request.user).values("name").first()
            )
            pig_form.cleaned_data["owner"] = user_breeding["name"]
            data = {
                "exhibitor_pig": pig_form.cleaned_data,
                "exhibitor_parents": [
                    {"id": i + 1, **form.cleaned_data}
                    for i, form in enumerate(parent_forms)
                ],
            }
    ...
    with open(json_file_path, "r") as json_file:
        for line in json_file:
            try:
                ...
                pig_nickname = (
                    row_data.get("data", {}).get("exhibitor_pig", {}).get("nickname")
                )
...
def add_parent_pig(pig_data, sex, mother, father):
    pig_info = Pig.objects.filter(
        name=pig_data["name"], nickname=pig_data["nickname"], breed=pig_data["breed"]
    ).first()
    if mother:
        mother_data = Pig.objects.filter(
            name=mother["name"], nickname=mother["nickname"], breed=mother["breed"]
        ).first()
    else:
        mother_data = None
    if father:
        father_data = Pig.objects.filter(
            name=father["name"], nickname=father["nickname"], breed=father["breed"]
        ).first()
    else:
        father_data = None
    if pig_info:
...
def display_waiting_pig_details(request, pig_id):
    ...
    if request.method == "POST":
        ...
        if pig_form.is_valid() and all(form.is_valid() for form in parent_forms):
            ...
            parents_complete = [
                parent_data[i]["name"] != ""
                and parent_data[i]["nickname"] != ""
                and parent_data[i]["breed"] is not None
                and parent_data[i]["colors"] != "['','','','','']"
                for i in range(14)
            ]
            for i in range(14):
            ...
            if (
                pig_info
                and pig_info.owner.all().exists()
                and pig_info.owner != pig_data.get("owner")
            ):
                print("Edytujący świnkę nie jest jej właścicielem")
                print("Nie można edytować nie swojej świnki")
                print(
                    "Należy wysłać użytkownikowi maila z informacją o właścicielu"
                )  # TODO
            else:
                if parents_complete[0] and parents_complete[1]:
                    if parents_complete[2] and parents_complete[3]:
                        ...
                        add_parent_pig(
                            parent_data[2], "Female", parent_data[6], parent_data[7]
                        )
                        add_parent_pig(
                            parent_data[3], "Male", parent_data[8], parent_data[9]
                        )
                    if parents_complete[4] and parents_complete[5]:
                        ...
                        add_parent_pig(
                            parent_data[4], "Female", parent_data[10], parent_data[11]
                        )
                        add_parent_pig(
                            parent_data[5], "Male", parent_data[12], parent_data[13]
                        )
                    add_parent_pig(
                        parent_data[0], "Female", parent_data[2], parent_data[3]
                    )
                    add_parent_pig(
                        parent_data[1], "Male", parent_data[4], parent_data[5]
                    )
                add_pig(pig_data, parent_data[0], parent_data[1])
            print("Można usunąć wiersz z pliku json")  # TODO
            try:
                pig_id = int(pig_id)
                with open("waiting_pig_list.json", "r") as json_file, open(
                    "temp_waiting_pig_list.json", "w"
                ) as temp_file:
                    for line in json_file:
                        try:
                            row_data = json.loads(line)
    ...
    return render(
        request,
        "waiting_pig_list_details.html",
        {"parent_forms": parent_forms, "pig_form": pig_form},
    )
